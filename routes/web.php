...
Route::get('repo-supplier-opt',function(Request $request){
   if($request->input('search')){
       ...
       $get = opt::whereNotNull('b_case')->where('w_vendor', 'LIKE', '%' . $search . '%')
       ->orWhere('code',$search)->get();
   }else{
       $search = '';
       $get = opt::whereNotNull('b_case')->get();
   }
   return view('repo_supplier_opt',compact('get'));
})->middleware('auth','per1');
Route::get('callcenter-copy',function (){
$free = adahyt::where('id','!=',0)->sum('free');
 ...
 $reservation = adahyt::where('id','!=',0)->sum('reservation');
  $followers = follower::all();
        ...
        $get = reservation::where('gov_type',12)->orderBy('code','ASC')->get();;
    return view('callcenter-copy',compact('get','free','days2','reservation','followers'));
});
Route::get('safe', function () {
   $safes =DB::table('safe_transactions')->paginate(15);
    return view('safe',compact('safes'));
})->middleware('auth','per1');

Route::post('new_safe', function (Request $request) {
   DB::beginTransaction();
   ...
   // Mark all financial_transactions for this agent as returned
   $checkMount = $validated['total'] - $validated['amount']; // Since insert() returns bool, not an object, set to 0 or adjust logic as needed
...
// ->where('agent_name', $validated['source'])
// //   ->where('returned_to_main_safe', 0)
// ->update(['amount'=>$validated['amount']- $validated['amount'],]);
// }
   return redirect()->back()->with('success', 'تم إضافة دفعة بنجاح');
})->middleware('auth','per1');

Route::post('push_safe', function (Request $request) {
   $validated = $request->validate([
      'amount' => 'required|numeric|min:1',
   ]);
...
Route::get('checks',function (Request $request){
   if($request->input('search')){
       $search = htmlspecialchars($request->input('search'));
       $checks = DB::table('financial_transactions')
           ->where('agent_name', 'LIKE', '%' . $search . '%')
           ->paginate(15);
   }else{
       $search = '';
       $checks = DB::table('financial_transactions')->paginate(15);
   }

return view('checks',compact('checks'));
})->middleware('auth','per1');

Route::post('new_check',function (Request $request){
   $validated = $request->validate([
      'agent_name' => 'required|string|max:255',
      'amount' => 'required|numeric|min:1',
   ]);
   $totalSafe = DB::table('safe_transactions')->where('type', 'استلام نقدية')->sum('amount') - DB::table('safe_transactions')->where('type', 'صرف نقدية')->sum('amount') ;
   // $totalDismissed = $totalSafe - $request->input('amount');
   if ($totalSafe > $validated['amount']) {
      ...
      return redirect()->back()->with('fail', 'لا يوجد رصيد كافٍ في الخزنة الرئيسية لإضافة الدفعة');
   }
})->middleware('auth','per1');

Route::post('checks-delete',function (Request $request){
$id = $request->input('id');
$check = DB::table('financial_transactions')->where('id', $id)->first();
if($check  && $check->returned_to_main_safe === 1){
    DB::table('financial_transactions')->where('id', $id)->delete();
    return redirect()->back()->with('success', 'تم حذف الدفعة بنجاح');
}else{
    return redirect()->back()->with('fail', 'قفل مع الخزنة الرئيسية الاول ');
}
})->middleware('auth','per1');

Route::get('/card_adahy', function (Request $request) {
   if($request->input('search')){
       $search = htmlspecialchars($request->input('search'));
       $get = DB::table('adahyt')->where('code', $search)->get();
   }else{
       $search = '';
       $get = DB::table('adahyt')->where('gov',12)->get();
   }
return view('card_adahy',compact('get'));
})->middleware('auth','per1');

Route::get('/card_adahy_mani', function (Request $request) {
   if($request->input('search')){
       $search = htmlspecialchars($request->input('search'));
       $get = DB::table('adahyt')->where('code', $search)->get();
   }else{
       $search = '';
       $get = DB::table('adahyt')->where('gov',24)->get();
   }
return view('card_adahy',compact('get'));
})->middleware('auth','per1');
...
Route::get('/liven/', function () {
 ...
 $get = 0;
 //liven
      return view('liveScan', compact('get'));
...
Route::get('/export_excel_supplier', [AccountController::class, 'export_excel_supplier'])->name('export_excel_supplier');
Route::get('/export_excel_times', [AccountController::class, 'export_excel_times'])->name('export_excel_times');

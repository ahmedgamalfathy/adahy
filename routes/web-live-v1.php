...
})->where('emp','!=','website')->where('gov_type',12)->orderBy('code','ASC')->paginate('30');
...
Route::get('/weight_edit', function (Request $request) {
 ...
 $adahy_type = adahy_type::all();
  ...
  Route::get('/butcher_s', function (Request $request) {
    $id = (int) htmlspecialchars($request->code);
    if(!empty($id)){
     $get = opt::where('code',$id)->whereIN('type',[1,2,3,4])->get();   
    }else{
    $get = opt::whereIN('type',[1,2,3,4])->get();
    }
    $sak = sak::all();
    $days = days::all();
    $followers = follower::all();

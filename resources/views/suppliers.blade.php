    ...
    ***********************************-->
    ...
    font-weight: bold;">
		   ...
		   <!-- Button trigger modal -->
                        @if(DB::table('per')->where('u_id',Auth::user()->id)->where('page','new_supplier')->count() >0)
                                ...
                                <form method="post" action="new_supplier">
                                    ...
                                    <div class="modal-body" style="direction: rtl; font-size: 18px; padding: 25px; font-weight: bold;">
                                        <div class="mb-12 col-md-12">
                                            ...
                                            <select class="form-control @error('name') is-invalid @enderror" name="name" required>
                                                <option value="">اختر المورد</option>
                                                @foreach($vendors as $vendor)
                                                    <option value="{{ $vendor->name }}" {{ old('name') == $vendor->name ? 'selected' : '' }}>
                                                        {{ $vendor->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                      ...
                                      <div class="mb-12 col-md-12">
                                            ...
                                            <input type="number" step="any" class="form-control @error('count') is-invalid @enderror" name="count" value="{{ old('count') }}" placeholder="العدد " required>
                                            @error('count')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                        ...
                                        <div class="mb-12 col-md-12">
                                            ...
                                            <input type="number" step="any" class="form-control @error('total_weight') is-invalid @enderror" name="total_weight" value="{{ old('total_weight') }}" placeholder="الوزن الاجمالي" required>
                                            @error('total_weight')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                        ...
                                        <div class="mb-12 col-md-12">
                                            ...
                                            <input type="number" step="any" class="form-control @error('price') is-invalid @enderror" name="price" value="{{ old('price') }}" placeholder="السعر قائم" required>
                                            @error('price')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                        ...
                                        {{-- <div class="mb-12 col-md-12">
                                            ...
                                            <input type="date" class="form-control @error('created_at') is-invalid @enderror" name="created_at" value="{{ old('created_at') }}" required>
                                            @error('created_at')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                            ...
                                            <div class="modal-content">
                                                <form method="post" action="edit_supplier">
                                                    ...
                                                    <div class="modal-body" style="direction: rtl; font-size: 18px; padding: 25px; font-weight: bold;">
                                                        ...
                                                        {{-- <div class="mb-12 col-md-12">
                                                            <label class="form-label">تاريخ الدفعة*</label>
                                                            <input type="date" class="form-control @error('created_at') is-invalid @enderror" name="created_at" id="edit_created_at">
                                                            @error('created_at')
                                                            <span class="invalid-feedback" role="alert">
                                                                <strong>{{ $message }}</strong>
                                                            </span>
                                    ...
                                    <table class="table table-striped table-responsive-sm" style="width: 100%;">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{$g->type}}</td>
                                                <td>{{$g->count}}</td>
                                                <td>{{ number_format($g->total_weight,2) }}</td>
                                                <td>{{$g->price}}</td>
                                                <td>{{$g->total}}</td>
                                                <td>{{$g->created_at}}</td>
                                                <td>
                                                    @if (DB::table('per')->where('u_id',Auth::user()->id)->where('page','edit_supplier')->count() >0)
                                                        <a href="#" 
                                                        data-bs-toggle="modal" 
                                                        data-bs-target="#Modaledit" 
                                                        class="btn btn-primary c_edit shadow btn-xs sharp me-1" 
                                                        data-id="{{$g->id}}" 
                                                        data-name="{{$g->name}}" 
                                                        data-type="{{$g->type}}" 
                                                        data-count="{{$g->count}}" 
                                                        data-total_weight="{{$g->total_weight}}" 
                                                        data-price="{{$g->price}}"
                                                        >
                                                ...
                                                <td>
                                                    @if (DB::table('per')->where('u_id',Auth::user()->id)->where('page','delete_supplier')->count() >0)
                                                        <a href="#" data-bs-toggle="modal" data-bs-target="#Modaldel" class="btn btn-danger c_del shadow btn-xs sharp me-1" data-id="{{$g->id}}" data-name="{{$g->name}}" data-note="{{$g->count}}" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>  
                                                    @endif
                                                </td>
                                                <td>
                                                    @if (DB::table('per')->where('u_id',Auth::user()->id)->where('page','repo_supplier')->count()>0)   
                                                        <a href="/suppliers/{{$g->name}}">
                                                            <div class="icon me-3">

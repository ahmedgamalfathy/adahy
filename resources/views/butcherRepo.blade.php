                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-responsive-sm" style="width: 100%;">
                                        ...
                                        <tbody>
                                            @foreach($get as $g)
                                            @php
                                            $countCow=DB::table('opt')->where('b_butcher',$g->name)->where('adahy','بقرى')->count();
                                            // $wightCow=DB::table('opt')->where('b_butcher',$g->name)->where('adahy','بقرى')->where('days','اليوم الأول')->sum(DB::raw('CAST(w_weight AS DECIMAL(10,4))'));
                                            $countBuf=DB::table('opt')->where('b_butcher',$g->name)->where('adahy','جمسى')->count();
                                            // $wightBuf=DB::table('opt')->where('b_butcher',$g->name)->where('adahy','جمسى')->sum(DB::raw('CAST(w_weight AS DECIMAL(10,4))'));
                                            $countShe=DB::table('opt')->where('b_butcher',$g->name)->where('adahy','خراف')->count();
                                            $countGoa=DB::table('opt')->where('b_butcher',$g->name)->where('adahy','ماعز')->count();
                                            $countEx=DB::table('opt')->where('b_butcher',$g->name)->where('adahy','من الخارج')->count();
                                            @endphp
                                            <tr>
                                                <td>{{$g->id}}</td>
                                                <td>{{$g->name}}</td>

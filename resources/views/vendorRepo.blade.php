                                    ...
                                    <div class="modal fade" id="exampleModalCenter">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                                <form method="post" action="new_vendor">
                                        @csrf
...
{{--                                     
                                    ...
                                    <div class="modal fade" id="Modaledit">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                                <form method="post" action="edit_vendor">
                                        @csrf
                        ...
                        <input type="text"  id="edit_note" class="form-control @error('note') is-invalid @enderror" name="note" value="{{ old('note') }}" placeholder="ملاحظات" Required>
                                ...
                                @enderror
                                    ...
                                    <div class="modal fade" id="Modaldel">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                                <form method="post" action="del_vendor">
                                        @csrf
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-responsive-sm" style="width: 100%;">
                                        ...
                                        <tbody>
                                            @foreach($get as $g)
                                            @php
                                            $countCow=DB::table('opt')->where('w_vendor',$g->name)->where('adahy','بقرى')->where('days','اليوم الأول')->count();
                                            $wightCow=DB::table('opt')->where('w_vendor',$g->name)->where('adahy','بقرى')->where('days','اليوم الأول')->sum(DB::raw('CAST(w_weight AS DECIMAL(10,4))'));
                                            $countBuf=DB::table('opt')->where('w_vendor',$g->name)->where('adahy','جمسى')->where('days','اليوم الأول')->count();
                                            $wightBuf=DB::table('opt')->where('w_vendor',$g->name)->where('adahy','جمسى')->where('days','اليوم الأول')->sum(DB::raw('CAST(w_weight AS DECIMAL(10,4))'));
                                            $countShe=DB::table('opt')->where('w_vendor',$g->name)->where('adahy','خراف')->where('days','اليوم الأول')->count();
                                            $countGoa=DB::table('opt')->where('w_vendor',$g->name)->where('adahy','ماعز')->where('days','اليوم الأول')->count();
                                            @endphp
                                            <tr>
                                                <td>{{$g->id}}</td>
                                                <td>{{$g->name}}</td>
                                                <td>{{ $countCow }}</td>
                                                <td>{{ number_format($wightCow, 2) }}</td>
                                                <td>{{ $countBuf }}</td>
                                                <td>{{number_format($wightBuf,2)  }}</td>
                                                <td>{{ $countShe }}</td>
                                                <td>{{ $countShe }}</td>
                                                
                                               {{-- <td>{{@tr_vendor::where('treasury_id',$g->id)->orderBy('id','DESC')->first()->total}}</td> --}}

    ...
    ***********************************-->
    ...
    font-weight: bold;">
		   ...
		   <!-- Button trigger modal -->
                          @if (DB::table('per')->where('u_id',Auth::user()->id)->where('page','new_check')->count() > 0)
                    ...
                    <div class="modal fade" id="exampleModalCenter">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <form method="post" action="new_check">
                                    @csrf
                                    <div class="modal-header">
                                        <h5 class="modal-title"> اضافة دفعة  </h5>
                                    ...
                                    <div class="modal-body" style="direction: rtl; font-size: 18px; padding: 25px; font-weight: bold;">
                                        <div class="mb-12 col-md-12">
                                            ...
                                            <select class="form-control @error('agent_name') is-invalid @enderror" name="agent_name" required>
                                                <option value="">اختر اسم المحاسب</option>
                                                <option value="سعدالدين" {{ old('agent_name') == 'سعدالدين' ? 'selected' : '' }}>سعدالدين</option>
                                                <option value="أحمد سالم" {{ old('agent_name') == 'أحمد سالم' ? 'selected' : '' }}>أحمد سالم</option>
                                                <option value="محمد عبدالكريم" {{ old('agent_name') == 'محمد عبدالكريم' ? 'selected' : '' }}>محمد عبدالكريم</option>
                                                <option value="سالم حسابات" {{ old('agent_name') == 'سالم حسابات' ? 'selected' : '' }}>سالم حسابات</option>
                                                <option value="محمد حسابات" {{ old('agent_name') == 'محمد حسابات' ? 'selected' : '' }}>محمد حسابات</option>
                                            </select>
                                            @error('agent_name')
                                      ...
                                      <div class="mb-12 col-md-12">
                                            ...
                                            <input type="number" step="any" class="form-control @error('amount') is-invalid @enderror" name="amount" value="{{ old('amount') }}" placeholder="المبلغ " required>
                                            @error('amount')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                            ...
                            <div class="card-body" dir="rtl">
                                <div class="table-responsive">
                                    <table class="table table-striped table-responsive-sm">
                                        ...
                                        <tbody>
                                            @php
                                                $currentAgent = auth()->user()->email ?? null;
                                                $agents = collect([$currentAgent])->filter()->unique();
                                            @endphp
		    ...
		    <div class="row">
                    <div class="col-12" style="direction: rtl;">
                        <div class="card">
                                ...
                                <form method="GET" action="" class="mb-3">
                                    <div class="row justify-content-center">
                                        <div class="col-md-6 mb-2 mx-auto text-center">
                                            <input type="text" name="search" class="form-control" placeholder="اسم المحاسب" value="{{ request('search') }}">
                                        </div>
                                        <div class="col-md-3 mb-2 d-flex justify-content-center gap-2 mx-auto">
                                    ...
                                    <table class="table table-striped table-responsive-sm" style="width: 100%;">
                                        <thead>
                                            <tr>
                                                <th>كود</th>
                                                <th>المحاسب</th>
                                                <th>رقم حجز المضحي</th>
                                                <th>كود الاضحية</th>
                                                <th>نوع المعاملة </th>
                                                <th>المبلغ</th>
                                                 <th>تم تصفية المبلغ مع الخزنه؟</th>
                                        ...
                                        <tbody>
                                            @foreach($checks as $g)
                                            <tr {{ $g->returned_to_main_safe  == 1 ? 'style=background-color:#76ff03' : '' }}>
                                                <td>{{$g->id}}</td>
                                                <td>{{$g->agent_name }}</td>
                                                <td>{{$g->res_rec  ?? "استلام من الخزنة"}}</td>
                                                <td>{{$g->adahy_code ?? "استلام من الخزنة" }}</td>
                                                <td>{{$g->type}}</td>
                                                <td>{{$g->amount}}</td>
                                                <td>{{ $g->returned_to_main_safe  == 0 ? "لا" : "نعم" }}</td>
                                                <td>{{$g->created_at}}</td>

                                        <td>
                                            @if (DB::table('per')->where('u_id', Auth::user()->id)->where('page', 'checks-delete')->count() > 0)
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#Modaldel" class="btn btn-danger c_del shadow btn-xs sharp me-1" data-id="{{$g->id}}" data-name="{{$g->agent_name}}"  class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i><a>                                                       
                                            @endif
                                        </td>
                                                <td>
                                                    <a href="#">
                         ...
                         {{$checks->withQueryString()->links("pagination::bootstrap-4")}}
                                        </tbody>
                                        <tfoot>
                                            <tr>
                                                <th>كود</th>
